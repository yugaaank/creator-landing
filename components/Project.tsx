...
export default function Project({ item }: { item: any }) {
	...
	return (
		<section className="w-full bg-[#481E14] rounded-t-[20px]">
						...
						<div
							...
							{item.title.split("").map((item: any, i: number) => (
								<motion.span
									...
									className="text-[165px] leading-none inline-block uppercase font-FoundersGrotesk text-[#F2613F] font-bold text-center"
									key={i}>
									{item}
								</motion.span>
							))}
					...
					<div className="flex items-center gap-[10px] mt-[20px] flex-wrap">
						{item.links.map((link: any) => (
							<div
								...
								key={link.id}>
								<Link
									className="text-[18px] leading-[18px] font-NeueMontreal text-white uppercase group-hover:text-[#0C0C0C] transition-all duration-300 ease-in-out hover:text-[#0C0C0C]"
									href={link.href}>
									<Rounded
										className="py-[10px]"
										backgroundColor="#fff">
			...
			<div className="w-full flex pt-[50px] pb-[200px] justify-center">
				<div className="flex items-center justify-between bg-white cursor-pointer rounded-full group">
					<Link
						className="text-[19px] text-[#0C0C0C] uppercase font-normal font-NeueMontreal"
						href={"/presentation"}>
						<Rounded
							className="py-[10px]"
							backgroundColor="#0C0C0C">
							<p className="text-[#0C0C0C] z-10 px-[15px] ml-[30px] py-[10px] group-hover:text-white">
								view all case studies
							</p>
							<div className="bg-[#0C0C0C] p-[15px] rounded-full scale-[0.3] mr-[17px] group-hover:scale-[1] transition-all z-10 text-[#0C0C0C] group-hover:bg-white  duration-300 ease-in-out">
								<ArrowUpRight
									strokeWidth={1.5}
									size={25}
								/>
